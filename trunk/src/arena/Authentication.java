...
public class Authentication extends HttpServlet {
	/*
	 * -----------------------------------------------Authentication----------------------------------------------- 
	 * To check if a single user is exists |GET: http://localhost:8080/TheArenaServler/Authentication?email="SOME_EMAIL"&password="SOME_PASSWORD" 	|Done
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(checkHeader(request)) {
			...
			if (signInUser != null) {
				...
				res.put("Error", "User is not exhists");
				response.getWriter().append(res.toJSONString());
			}
		}else {
			res.clear();
